...
CMD ["sh", "-c", "mkdir -p /logs && until nc -z 127.0.0.1 $SOCKS_PORT; do echo 'Waiting for Tor SOCKS...'; sleep 3; done && while true; do python3 -u thor_main.py -T >> /logs/sessions.log 2>&1; sleep 5; done"]
